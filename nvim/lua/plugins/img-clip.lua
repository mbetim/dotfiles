...
return {
	...
	keys = function()
		...
		local is_path_url = function(image_path)
			return string.sub(image_path, 1, 4) == "http"
		end

		local exec_command_on_image = function(command_prefix)
			-- Get the image path
			...
			if is_path_url(image_path) then
			...
			local command = command_prefix .. vim.fn.shellescape(absolute_image_path)
			return vim.fn.system(command)
		end

		return {
			-- suggested keymap
			{ "<C-a>", "<cmd>PasteImage<cr>", desc = "Paste image from system clipboard" },
			{
				"<leader>io",
				function()
					local success = exec_command_on_image("open -a Preview ")

					if success ~= false then
						print("Opened image in Preview")
					else
						print("Failed to open image in Preview:" .. success)
					end
				end,
			...
			},
			{
				"<leader>if",
				function()
					local success = exec_command_on_image("open -R ")

					if success ~= false then
						print("Opened image in Finder")
					else
						print("Failed to open image in Finder: " .. success)
					end
				end,
				desc = "Open image in Finder",
			},
			{
				"<leader>ir",
				function()
